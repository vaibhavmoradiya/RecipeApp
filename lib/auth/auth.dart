...
abstract class BaseAuth {
  ...
  Future<String> signUpEmailPassword(User usuario);//model/usuarios.dart
  Future<void>   signOut();
...
class Auth implements BaseAuth{
  ...
  Future<String> signUpEmailPassword(User usuarioModel) async {
    ...
    await user.sendEmailVerification().then((onValue) => print('Verification email sent'))
        .catchError((onError) => print('Verification Email Error: $onError'));

    await Firestore.instance.collection('users').document('${user.uid}').setData({
      'Name': usuarioModel.name,//name
      'MobileNo': usuarioModel.mobileno,//phone
      'Email': usuarioModel.email,
      // 'City': usuarioModel.city,//city
      'Address': usuarioModel.address
    })
        .then((value) => print('User registered in the database'))
        .catchError(
            (onError) => print('Error in registering the user in the database'));
    return user.uid;
  }
